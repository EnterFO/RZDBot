...
function main()  
  ...
  sampRegisterChatCommand("rzdbot", function() act = not act sampAddChatMessage(act and "{8B00FF}[RZDBot]{E6A8D7} Бот подключился к игре и принял задачу..." or "{8B00FF}[RZDBot]{E6A8D7} Вы успешно выключили бота!" ) end)
...
function onReceivePacket(id, bs)  
    ...
    if str:find("window.executeEvent%('cef.hud.updatePlayerFood'") then
        local foodValue = tonumber(str:match("%[(%d+%.%d+)%]"))
        if foodValue and foodValue < 45 then
            sampSendChat("/eat")
        end
    end
